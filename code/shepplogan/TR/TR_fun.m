...
p0_magnitude = 2;
path = ['D:\matlab_paper\shepp_logan\shepp_logan_pic\',num2str(j),'.jpg']
p0 = imread(path); % 读取图片
...
imagesc(kgrid.y_vec * 1e3, kgrid.x_vec * 1e3, p0 + cart2grid(kgrid, cart_sensor_mask), [0, 1]);
colormap("gray");
ylabel('x-position [mm]');
xlabel('y-position [mm]');
axis image;
path = ['D:\matlab_paper\shepp_logan\pressure_sensor\',num2str(j),'.jpg'];
export_fig(path,gcf);

% plot the initial pressure
figure;
set(gcf,"position",[400, 400, 330, 329]);
imagesc(kgrid.y_vec * 1e3, kgrid.x_vec * 1e3, p0, [0, 1]);
colormap("gray");
% ylabel('x-position [mm]');
% xlabel('y-position [mm]');
axis image;
...
figure;
set(gcf,"position",[400, 400, 330, 329]);
imagesc(sensor_data, [0, 1]);
colormap("gray");
ylabel('Sensor Position');
xlabel('Time Step');
colorbar;
path = ['D:\matlab_paper\shepp_logan\sensor_data\',num2str(j),'.jpg'];
export_fig(path,gcf);

% plot the reconstructed initial pressure using the interpolated data
figure;
set(gcf,"position",[400, 400, 330, 329]);
imagesc(kgrid_recon.y_vec * 1e3, kgrid_recon.x_vec * 1e3, p0_recon_interp, [0, 1]);
colormap("gray");
% ylabel('x-position [mm]');
% xlabel('y-position [mm]');
axis image;
axis off;
path = ['D:\matlab_paper\shepp_logan\shepp_logan_rs2\',num2str(j),'.jpg'];
export_fig(path,gcf);
